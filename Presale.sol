...
/**
 * @dev Contract module which provides access control mechanism, where there
 * is an account (an owner) that can be granted exclusive access to specific
 * functions.
 *
...
abstract contract Ownable2Step is Ownable {
    ...
    /**
     * @dev Cancels the ownership transfer to the pending owner.
     */
...
contract VLTCSale is Ownable2Step, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_aggregatorAddress != address(0), "Zero aggregator address");
        require(_receiverWallet != address(0), "Zero receiver wallet");
    ...
    function addAllowedPayTokens(address[] memory _tokens) external onlyOwner {
        for (uint i = 0; i < _tokens.length; i++) {
            require(_tokens[i] != address(0), "Zero address in tokens array");
    ...
    function transferFunds(address _token, uint256 _amount) internal {
        if (_token == address(0)) {
            (bool success, ) = payable(receiverWallet).call{value: _amount}("");
    ...
    function purchaseTokens(
        ...
        uint256 purchaseValueUSD = (saleTokenAmt * priceInUSD) /
            (10 ** saleTokenDec);
        require(
            purchaseValueUSD >= minBuyAmountUSD,
            "Below minimum buy amount"
        );
        require(
            purchaseValueUSD <= maxBuyAmountUSD,
